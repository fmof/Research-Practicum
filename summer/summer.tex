...
\begin{abstract}
\todo{provide an overview of what you learned: basic calculus, statistics, NLP, etc.}  A review of basic knowledge about the field of computation linguistics and a description of what I've accomplished over the summer.
\end{abstract}
...
N-grams are a very commonly used tool in computational linguistics for modeling language. N-grams work by assigning a sequence of tokens (words, punctuation, etc.) of length $N$ a probability of occurring. This probability is equal to \todo{replace this sentence w/: Each word $w_i$ is assigned a probability conditioned on the previous $i-1$ words. This probability can be written as}
...
\todo{say ``word,'' and put ``token'' in parentheses} Here, $w$ represents a token and the notation $w_{1:i-1}$ is equivalent to the sequence of tokens $w_1, w_2, \cdots, w_{i-1}$. Additionally, $V$ represents the vocabulary set, with the sum iterating over each type $v$. \todo{say ``unique word type''}

\todo{before this paragraph, you should mention what the probability of an entire tweet is. It'd be good to include the generative story (and explain what that means). Make sure you mention that you don't actually believe that data were generated that way; it's an approximation} If you let
...
\todo{better transition, something like: ``The denominator $c(w_{1:i-1})$ is the marginal count.'' Then transition into the formula. This may mean rearranging the sentences in this paragraph.} $\sum\limits_{v \in V} c\left( w_{i-1}, v \right) = c\left( w_{i-1}\right)$ due to the fact that every time a two token sequence occurs, that first token in the sequence also occurs. Therefore, the sum of all the two token counts is equal to the one token count, called the marginal count. In a more general form,

$$ c\left(w_1, w_2, \cdots, w_i\right) =  \sum\limits_{v \in V} c\left(w_1, w_2, \cdots, w_i, v\right) $$

An example use of an N-gram model would be when you want to predict the next word in a sequence. If I have the sequence "How are", you could use an N-gram trained on other data to try and predict the next word by iterating over your vocabulary and choosing the highest probability \todo{make this display, not inline} $\argmax\limits_{w \in V} P\left( w \; \vert \; \text{How}, \text{are} \right)$. This could, for example, be "you" where $P\left( \text{you} \; \vert \; \text{How}, \text{are} \right) = 0.7$. In this example, the N-gram is a trigram (3-gram) as it uses 3 tokens in calculating the probability.
...
The derivative of an additively smoothed bigram log-likelihood can be derived as follows \todo{this is a bit awkward, since you haven't introduced additive smoothing yet. I'd suggest just keeping the first equation, and maybe moving 2-4 to an appendix}
...
This problem of data sparsity can be seen in the following graph \todo{explain more about this graph: what is the x-axis, what is the y-axis?}
...
The reason that interpolation is useful is it averages multiple models that are all able to partially explain the generative story of the text. \todo{add a footnote citation saying that this example is either due to p.c. (personal correspondence) w/ me, or just cite the github page}

For example, say there are two probability distributions, $\mathcal{F} = N\left(-3, 1\right)$ and $\mathcal{G} = N\left(3, 1 \right)$, both normal distributions but with different means. \todo{switch this sentence and the next} There is also a weighted coin that has a $0.3$ chance of landing heads up (Bernoulli(.7)). Let $X$ be equal to 2000 data points drawn from the hierarchal model $\mathcal{H}$. For each point, flip the coin and if it's heads, draw a point from $\mathcal{F}$, otherwise draw a point from $\mathcal{G}$

This produces the following histogram

\includegraphics[width=218px]{graphs/hist.png}

The normalized log-likelihood of $X$ scored with model $\mathcal{F}$ and $\mathcal{G}$ equals \todo{I'd remove the formulae here: put the scores inline}
...
which is significantly \todo{higher, not lower} lower. Interpolating multiple N-gram models works in a similar manner to lower normalized log-likelihoods by better describing the generative story of the text.
...
\todo{I think this paragraph can be cut (commented out). You've covered this info at various other points} Language models have several constraints. The largest of these is that language must be valid probability distributions, with all probabilities between 0 and 1 and summing to 1.
...
\todo{you can say this here, but this needs to be made clear very early on too} My main accomplishment over the summer was an implementation of an N-gram model in python. The code for it can be found on Github at \\
...
Here the relationship between the amount of additive smoothing on a model and it's normalized log-likelihood can be seen. \todo{explain what's happening here; what's the straight line; what do the different lines mean; what data did you use (how many tweets, how many tokens)?}
...
\todo{Describe what you're doing here, even as a sentence or two}
